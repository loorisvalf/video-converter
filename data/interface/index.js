  ...
  "command": {
    ...
    "run": function (command) {
      if (!config.running && config.worker.ready) {
        ...
        var files = [{"name": "input", "data": config.video.data}];
        var video = config.video.data && config.video.data.byteLength;
        var options = video ? {"type": "command", "arguments": args, "files": files} : {"type": "command", "arguments": args};
        /*  */
      ...
      "player": function (display) {
        ...
        if (display === "block" && config.file.output) {
          ...
          if (name && name.match(/\.jpeg|\.gif|\.jpg|\.png|\.bmp|\.tiff|\.tif/)) {
            ...
            config.element.preview.appendChild(img);
          } else {
    ...
    "init": async function () {
      ...
      config.worker.element.onmessage = function (e) {
        ...
        if (message.type === "start") {
          config.element.output.textContent = "Video Converter received a command.\n\n";
