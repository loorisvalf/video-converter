var config = {
  ...
  "load": function () {
    ...
    config.app.element.drop.video = document.getElementById("video-fileio");
    config.app.element.info.video = document.getElementById("video-fileinfo");
    /*  */
  ...
  "app": {
    ...
    "listener": {
      ...
      "loadend": function (e) {
        ...
        config.app.video.buffer = e.target.result;
        if (config.app.video.buffer) {
          const size = config.size(config.app.file.input.size);
          config.app.video.data = new Uint8Array(config.app.video.buffer);
          config.app.element.info.video.textContent = "File size: " + size;
          config.app.element.output.textContent += "> The " + config.app.file.input.name + " (" + size + ") file is ready, please continue." + "\n";
        }
      },
      "fileio": function () {
        config.app.element.drop.video.addEventListener("change", function (e) {
          if (e.target) {
            if (e.target.files) {
      ...
      "output": {
        ...
        "src": function () {
          ...
          if (ext) {
            ...
            config.app.create.output.blob = new Blob([data], {"type": "video/" + ext});
        ...
        "player": function (display) {
          ...
          if (display === "block" && config.app.file.output.name) {
            ...
            if (name && name.match(/\.jpeg|\.gif|\.jpg|\.png|\.bmp|\.tiff|\.tif/)) {
              ...
              config.app.element.preview.appendChild(video);
            }
            /*  */
            config.app.element.preview.children[1].src = config.app.video.src + "#t=0.5";
          }
        }
      }
    },
        ...
        if (config.app.worker.ready) {
          if (config.app.running === false) {
            ...
            try {
              ...
              if (args && args.length) {
                if (config.app.video.data) {
                  if (config.app.video.data.byteLength) {
                    if (command.indexOf(config.app.file.input.name) !== -1) {
                      config.app.file.output.name = args[args.length - 1];
                      await config.ffmpeg.core.writeFile(config.app.file.input.name, config.app.video.data);
                    }
                  } else {
                    config.app.file.reader.readAsArrayBuffer(config.app.file.input);
                    config.app.element.output.textContent += "> Reading the input file, please wait..." + "\n";
    ...
    "worker": {
      ...
      "init": async function () {
        try {
          config.app.element.output.textContent = "Video Converter is getting ready, please wait...\n";
          /*  */
            ...
            if (e) {
              if (e.progress === 1) {
                ...
                if (config.app.file.output.data) {
                  config.app.video.src = config.app.create.output.src();
                  if (config.app.video.src) {
